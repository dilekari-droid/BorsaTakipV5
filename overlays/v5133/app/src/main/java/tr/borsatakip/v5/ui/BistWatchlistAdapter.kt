...
class BistWatchlistAdapter(
    ...
    override fun onBindViewHolder(holder: H, position: Int) {
        ...
        holder.meta.text = stock?.let { s ->
            val mode = when {
                s.isRealtime -> "GERÇEK ZAMANLI"
                (s.delaySeconds ?: 0) > 0 -> "${s.delaySeconds}s GECİKMELİ"
                else -> "DOĞRULANMIŞ VERİ"
            }
            "Kaynak: ${s.source} • $mode"
        } ?: "Fiyat verisi alınamadı • Yenile ile tekrar dene"
