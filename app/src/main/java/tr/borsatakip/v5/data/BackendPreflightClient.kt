...
class BackendPreflightClient(context: Context) {
    ...
    enum class FailureKind {
        ...
        AUTH_ERROR, HTTP_ERROR, SERVER_ERROR, NETWORK_TIMEOUT, DNS_ERROR, TLS_ERROR,
    ...
    suspend fun check(): Result = withContext(Dispatchers.IO) {
        ...
        if (!historyOk || !json.optBoolean("ok", false)) return@withContext fail(FailureKind.HISTORY_ERROR, check("history")?.optString("message").orEmpty().ifBlank { "BIST history doğrulanamadı." }, started, true, true, true, true, provider = provider, symbolCount = symbolCount, sampleSymbol = sample)

        // VİOP readiness: mevcut gerçek endpointler üzerinden ayrı doğrulama.
        val contractsResult = requestJson(base, "/v1/viop/contracts")
        if (!contractsResult.ok) return@withContext fail(FailureKind.VIOP_CONTRACTS_ERROR, "VİOP contract universe başarısız • ${contractsResult.detail}", started, true, true, true, true, true, provider = provider, symbolCount = symbolCount, sampleSymbol = sample)
        val contractsJson = contractsResult.json ?: return@withContext fail(FailureKind.VIOP_CONTRACTS_ERROR, "VİOP contract cevabı geçersiz.", started, true, true, true, true, true, provider = provider, symbolCount = symbolCount, sampleSymbol = sample)
        val contracts = contractsJson.optJSONArray("items")
        val viopCount = contracts?.length() ?: 0
        if (viopCount <= 0) return@withContext fail(FailureKind.VIOP_CONTRACTS_ERROR, "Aktif VİOP sözleşme evreni boş.", started, true, true, true, true, true, provider = provider, symbolCount = symbolCount, sampleSymbol = sample)
        val viopSymbol = contracts?.optJSONObject(0)?.optString("symbol")?.takeIf { it.isNotBlank() }
            ?: return@withContext fail(FailureKind.VIOP_CONTRACTS_ERROR, "VİOP örnek sözleşme sembolü alınamadı.", started, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample)
        val encoded = URLEncoder.encode(viopSymbol, "UTF-8")

        val viopQuote = requestJson(base, "/v1/viop/quote/$encoded")
        if (!viopQuote.ok) return@withContext fail(FailureKind.VIOP_QUOTE_ERROR, "VİOP quote başarısız • ${viopQuote.detail}", started, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)
        val quoteJson = viopQuote.json ?: return@withContext fail(FailureKind.VIOP_QUOTE_ERROR, "VİOP quote cevabı geçersiz.", started, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)
        if (!quoteFresh(quoteJson)) return@withContext fail(FailureKind.STALE_DATA, "VİOP quote güncel/realtime değil.", started, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)

        val viopHistory = requestJson(base, "/v1/viop/history/$encoded?range=1y&interval=1d")
        if (!viopHistory.ok) return@withContext fail(FailureKind.VIOP_HISTORY_ERROR, "VİOP history başarısız • ${viopHistory.detail}", started, true, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)
        val historyJson = viopHistory.json ?: return@withContext fail(FailureKind.VIOP_HISTORY_ERROR, "VİOP history cevabı geçersiz.", started, true, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)
        val candleCount = historyJson.optInt("candleCount", historyJson.optJSONArray("candles")?.length() ?: 0)
        if (candleCount < 220) return@withContext fail(FailureKind.VIOP_HISTORY_ERROR, "VİOP history yetersiz: $candleCount mum; en az 220 gerekli.", started, true, true, true, true, true, true, true, provider = provider, symbolCount = symbolCount, viopContractCount = viopCount, sampleSymbol = sample, sampleViopSymbol = viopSymbol)
        ...
        Result(true, FailureKind.NONE,
            "Provider READY • Health ✓ • Authentication ✓ • BIST Symbols ✓ • BIST Quote ✓ • BIST History ✓ • VİOP Contracts ✓ • VİOP Quote ✓ • VİOP History ✓",
            true, true, true, true, true, true, true, true,
            symbolCount, viopCount, sample, viopSymbol, provider, System.currentTimeMillis() - started)
    }
