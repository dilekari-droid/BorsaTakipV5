...
async function handlePreflight(res) {
  ...
  try {
    ...
    send(res, 200, {
      ...base,
      ok,
      quote:{ok:quoteOk, message: quoteOk ? 'Current-session quote is live.' : `Upstream quote is not live/current-session (age=${q.delaySeconds}s).`},
...
async function route(req, res) {
  ...
  if (m) {
    try {
      ...
      const result=await yahooChart(symbol,{interval,period1:Math.floor(from/1000),period2:Math.floor(to/1000)+1});
